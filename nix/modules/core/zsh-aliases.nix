...

{
  programs.zsh.shellAliases = {
    ...
    aic = ''agy --prompt "Analyze the current git diff under the current directory(pwd). Execute 'git add -A' and then execute 'git commit' with a concise message following the Conventional Commits specification (e.g., 'feat: ...', 'fix: ...'). Do NOT push to remote."'';
    aim = ''agy --prompt "Analyze all modified files under the current directory(pwd). Group them into logical sets based on functionality. For each set: 1. Execute 'git add' for those specific files only. 2. Execute 'git commit' with a descriptive Conventional Commit message. Repeat until all changes are committed. Do NOT push"'';
    gh = "bunx gh";
